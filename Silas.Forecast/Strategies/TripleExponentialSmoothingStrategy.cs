...
namespace Silas.Forecast.Strategies
{
    public class TripleExponentialSmoothingStrategy : IForecastStrategy
    {
        ...
        public ForecastEntry Forecast(IEnumerable<DataEntry> dataEntries, int period, dynamic strategyParameters)
        {
            ...
            if (_periodsPerSeason*_seasonsForRegression > dataEntries.Count())
                throw new ArgumentException(
                    "You need at least more seasons of data than SeasonsForRegression to build a data model.");
        ...
        /// </summary>
        /// <returns></returns>
        private double GenerateForecast(int startPeriod, int endPeriod, IEnumerable<DataEntry> dataEntries,
                                        IList<SeasonalIndex> seasonalIndices)
        {
            ...
            for (currPeriod = startPeriod + (_periodsPerSeason*2);
                 currPeriod < endPeriod;
                 currPeriod += _periodsPerSeason)
            {
        ...
        private IList<TripleExponentialEntry> BuildNextSeason(IList<TripleExponentialEntry> currSeason,
                                                              IEnumerable<DataEntry> dataEntries, int currPeriod)
        {
        ...
        private IList<SeasonalIndex> BuildSeasonalIndices(IEnumerable<DataEntry> dataEntries)
        {
            Regression(dataEntries.Select(e => e.Period).Take(_periodsPerSeason*_seasonsForRegression).ToArray(),
                       dataEntries.Select(e => e.Value).Take(_periodsPerSeason*_seasonsForRegression).ToArray(), out _a,
                       out _b);
